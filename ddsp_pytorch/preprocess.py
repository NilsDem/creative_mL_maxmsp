...
def preprocess(f, sampling_rate, block_size, signal_length, oneshot, device,
               **kwargs):
    x, sr = li.load(path=f, sr=sampling_rate)
...
def main():
    ...
    for i, f in enumerate(pb):
        ...
        x, p, l = preprocess(f, **config["preprocess"], device=device)
        signals.append(x)
        pitchs.append(p)
        loudness.append(l)
